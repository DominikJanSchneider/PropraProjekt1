...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		lblConfigPanel = new JLabel(
				"Sicherheitsunterweisung am Institut f\u00fcr Werkstofftechnik und Ger\u00e4tezentrum MNaF");
		...
		// Fill JTextAreas (Allgemeine Unterweisung, Laboreinrichtungen, Gefahrstoffe)
		// with data from selected row
		...
		spAllgemeineUnterweisungen = new JScrollPane(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
		...
		spLaboreinrichtungen = new JScrollPane(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		...
		spGefahrstoffe = new JScrollPane(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		...
		filterPanelGeraeteTab
				.setLayout(new MigLayout("", "[300,grow][300,grow][300,grow][300,grow]", "[][grow][][grow]"));
	...
	public void loadFilter(Object[][] filteredTable) {
		...
		for (int i = 0; i < filteredTable.length; i++) {
			...
			dtm.addRow(new Object[] { filteredTable[i][0], filteredTable[i][1], filteredTable[i][2],
					filteredTable[i][3], filteredTable[i][4], filteredTable[i][5], filteredTable[i][6],
					filteredTable[i][7], filteredTable[i][8], filteredTable[i][9], filteredTable[i][10],
					filteredTable[i][11], filteredTable[i][12], filteredTable[i][13], filteredTable[i][14] });
	...
	public void loadFilterGeraete(Object[][] filteredTable) {
		...
		for (int i = 0; i < filteredTable.length; i++) {
			...
			geraeteTableModel.addRow(new Object[] { filteredTable[i][0], filteredTable[i][1], filteredTable[i][2],
					filteredTable[i][3] });
		}
	...
	public void loadFilterRaeume(Object[][] filteredTable) {
		...
		for (int i = 0; i < filteredTable.length; i++) {
			...
			raeumeTableModel.addRow(new Object[] { filteredTable[i][0], filteredTable[i][1] });
		}
	...
	public void loadFilterGefahrstoffe(Object[][] filteredTable) {
		...
		for (int i = 0; i < filteredTable.length; i++) {
			...
			gefahrstoffeTableModel.addRow(new Object[] { filteredTable[i][0] });
		}
	}
	...
	public static void getPersonData() {
		...
		dtm = new DefaultTableModel(new Object[][] {},
				new String[] { "ID", "Name", "Vorname", "Datum", "Ifwt", "MNaF", "Intern", "Beschaeftigungsverhaeltnis",
						"Beginn", "Ende", "Extern", "E-Mail Adresse", "Allgemeine Unterweisung", "Laboreinrichtungen",
						"Gefahrstoffe" }) {
			private static final long serialVersionUID = 1L;
		...
		// wir müssen halt noch schauen ob sie direkt geladen werden soll, oder auf
		// Befehl des Users
		...
		try {
			...
			table.getColumnModel().removeColumn(table.getColumnModel().getColumn(12)); // make column invisible but
																						// still accessible
			table.getColumnModel().removeColumn(table.getColumnModel().getColumn(12)); // make column invisible but
																						// still accessible
			table.getColumnModel().removeColumn(table.getColumnModel().getColumn(12)); // make column invisible but
			...
			editorTable.getColumnModel().removeColumn(editorTable.getColumnModel().getColumn(12)); // make column
																									// invisible but
																									// still accessible
			editorTable.getColumnModel().removeColumn(editorTable.getColumnModel().getColumn(12)); // make column
																									// invisible but
	...
	public static void getGeraeteData() {
		geraeteTableModel = new DefaultTableModel(new Object[][] {},
				new String[] { "Ger\u00e4teID", "Name", "Beschreibung", "Raum" }) {
		 ...
		 * DBConnection.getGeraeteData()[i][1], DBConnection.getGeraeteData()[i][2],
		 * DBConnection.getGeraeteData()[i][3]
		 */
		Object[][] data = DBConnection.getGeraeteData();
		for (int i = 0; i < data.length; i++) {
			geraeteTableModel.addRow(new Object[] { data[i][0], data[i][1], data[i][2], data[i][3] });
		}
	...
	public static void getRaeumeData() {
		...
		for (int i = 0; i < data.length; i++) {
			raeumeTableModel.addRow(new Object[] { data[i][0], data[i][1] });
		}
	...
	public static void getGefahrstoffeData() {
		...
		for (int i = 0; i < data.length; i++) {
			gefahrstoffeTableModel.addRow(new Object[] { data[i][0] });
		}
	...
	/*
	 * public void loginToFront() { java.awt.EventQueue.invokeLater(new Runnable() {
	 * 
	 * @Override public void run() { login.toFront(); login.repaint();
	 * login.setLocation((getWidth()/2)-(login.getWidth()/2),
	 * (getHeight()/2)-login.getHeight()/2); } }); }
	 */
...
class ColorTable extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, java.lang.Object value, boolean isSelected,
		...
		if (!date.isEmpty()) {
			...
			daysDiff = CalcDateDiff.date(date); // check difference between given date and actual date in
												// CalcDateDiff-Class
